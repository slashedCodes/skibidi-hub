...
app.get('/api/video/:id', (req, res) => {
    if(!req.headers.authorization) {
        return res.sendFile(path.join(__dirname, path.join("www", path.join("assets", path.join("troll", "video.mp4")))))
    }

    if (fs.existsSync(path.join("videos", req.params.id + "/"))) {
        res.sendFile(path.join(__dirname, path.join("videos", path.join(req.params.id, "video.mp4"))));
    } else {
        res.sendStatus(404);
    }
})
...
app.get('/api/thumbnail/:id', (req, res) => {
    if(!req.headers.authorization) {
        let images = fs.readdirSync(path.join(__dirname, path.join("www", path.join("assets", "troll"))));
        res.sendFile(path.join(__dirname, path.join("www", path.join("assets", path.join("troll", images[getRandomInt(images.length - 1)])))));
        return
    }

    if (fs.existsSync(path.join("videos", req.params.id + "/"))) {
        res.sendFile(path.join(__dirname, path.join("videos", path.join(req.params.id, "thumbnail.jpg"))));
    } else {
        res.sendStatus(404);
    }
})
...
app.get('/api/videoInfo/:id', (req, res) => {
    client.from("videos").select().eq("id", req.params.id).then(data => {
        if(data.error) { res.sendStatus(400) } else if(data.status != 200) { res.sendStatus(data.status) }
...
app.get('/api/comments/:videoID', (req, res) => {
    client.from("comments").select().eq("video_id", req.params.videoID).then(data => {
        if(data.error) { res.sendStatus(400) } else if(data.status != 200) { res.sendStatus(data.status) }
...
app.get('/api/getAllVideos', (req, res) => {
    client.from("videos").select().then(data => {
        if(data.error) { res.sendStatus(400) } else if(data.status != 200) { res.sendStatus(data.status) }
...
app.post("/api/like/:id", async (req, res) => {
    if(!req.headers.authorization) return;
    const likesData = await client.from("videos").select("likes").eq("id", req.params.id)
    const likes = likesData["data"][0]["likes"] + 1
    
    const video = await client.from("videos").update({ likes: likes }).eq("id", req.params.id)
    res.send(video)
})
...
app.post("/api/dislike/:id", async (req, res) => {
    if(!req.headers.authorization) return;
    const dislikesData = await client.from("videos").select("dislikes").eq("id", req.params.id)
    const dislikes = dislikesData["data"][0]["dislikes"] + 1
    
    const video = await client.from("videos").update({ dislikes: dislikes }).eq("id", req.params.id)
    res.send(video)
})
